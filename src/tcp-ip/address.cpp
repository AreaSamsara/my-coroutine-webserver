...
namespace AddressSpace
{
	...
	bool Address::operator==(const Address &right_address) const
	{
		...
		return getAddress_length() == right_address.getAddress_length() && memcmp(getAddress(), right_address.getAddress(), getAddress_length()) == 0;
	}
	bool Address::operator!=(const Address &right_address) const
	{
	...
	bool Address::Lookup(vector<shared_ptr<Address>> &addresses, const string &host, int family, int type, int protocol)
	{
		...
		if (address_str.empty())
		{
			...
			// 否则说明该host地址不携带端口号，直接将地址设置为host地址（此时端口号字符指针为默认值NULL，getaddrinfo()函数的搜索将不考虑端口号）
			else
			{
				address_str = host;
			}
