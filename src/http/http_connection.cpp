...
namespace HttpConnectionSpace
{
													 ...
													 const map<string, string> &headers, const string &body)
	{
		...
		if (!socket)
		{
			return make_shared<HttpResult>(HttpResult::Error::CREATE_SOCKET_FAIL, nullptr, "create socket fail,address=" + address->toString() + " errno=" + to_string(errno) + " strerror" + string(strerror(errno)));
		}
	...
	void HttpConnectionPool::ReleasePtr(HttpConnection *ptr, HttpConnectionPool *pool)
	{
		...
		if (!ptr->getSocket()->isConnected() || ptr->getCreate_time() + pool->m_max_alive_time >= GetCurrentMS() || ptr->getRequest_count() >= pool->m_max_request)
		{
			delete ptr;
			--pool->m_total;
		}
