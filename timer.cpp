...
namespace TimerSpace
{
	...
	//bool Timer::Comparator::operator()(const shared_ptr<Timer>& left_timer, const shared_ptr<Timer>& right_timer)const
	//{
	...
	//		return false;
	//	}
	//	if (left_timer->m_execute_time < right_timer->m_execute_time)
	//	{
	//		return true;
	//	}
	//	if (left_timer->m_execute_time > right_timer->m_execute_time)
	...
	void TimerManager::getExpired_callbacks(vector<function<void()>>& expired_callbacks)
	{
		...
		if (!rollover && ((*m_timers.begin())->getExecute_time() > now_ms))	//new
		{
			return;
		}
